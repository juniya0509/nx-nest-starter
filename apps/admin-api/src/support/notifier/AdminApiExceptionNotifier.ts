...
import { AdminLogType } from '../logger/AdminApiExceptionLogger';

const THROTTLE_MS = 10 * 60 * 1000;

export class AdminApiErrorNotifier {
  ...
  private static init(): boolean {
    ...
    if (!webhookUrl) {
      if (!this.missingWebhookWarned) {
        console.warn('[AdminApiErrorNotifier] SLACK_SERVER_ERROR_WEBHOOK_URL not configured — skipping notifications');
  ...
  static async notify(log: AdminLogType): Promise<void> {
    ...
    try {
      await this.webhook!.send({
        attachments: [
          {
            color: 'danger',
            pretext: `:rotating_light: *[Admin API] HTTP ${log.status} 서버 오류*`,
      ...
      console.error('[AdminApiErrorNotifier] Failed to send slack notification:', err);
    }
  }
