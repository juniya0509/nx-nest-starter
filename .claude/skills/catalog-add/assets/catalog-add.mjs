 ...
 *
 * 동작:
 *   1. 사용자에게 대상 프로젝트, 의존성 타입, 패키지 이름을 묻음
 *   2. catalog 무결성 검증 (자기참조 항목 차단)
 *   3. 입력 패키지를 "이미 catalog에 있음" / "새 패키지"로 분리
 *   4. 새 패키지만 첫 번째 대상에 임시 설치 → 버전 추출
 *   5. pnpm-workspace.yaml의 catalog 섹션 업데이트
 *   6. 모든 대상 프로젝트의 package.json을 "catalog:" 참조로 통일
 *   7. pnpm install로 동기화
 *
 * 의존성:
 *   - yaml (devDependency)
 *   - @inquirer/prompts (devDependency)
 */
...
function validateVersion(pkg, version) {
  ...
  if (version.startsWith('workspace:')) {
    ...
    console.error('   각 프로젝트의 package.json에서 직접 "workspace:*"로 참조하세요.\n');
    return false;
  }
  if (version.startsWith('link:') || version.startsWith('file:')) {
    console.error(`\n❌ ${pkg}의 버전이 "${version}"입니다.`);
    console.error('   로컬 링크/파일 참조는 catalog로 관리할 수 없습니다.\n');
    return false;
  }
  return true;
}
...
async function main() {
  ...
  if (!existsSync(yamlPath)) {
    console.error(`❌ ${yamlPath}이 없습니다. 워크스페이스 루트에서 실행하세요.`);
    process.exit(1);
  }
  ...
  // 9. 모든 대상 프로젝트의 package.json에 "catalog:" 참조 추가
